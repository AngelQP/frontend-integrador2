...
export function addMilliseconds(value: Date, milliseconds: number) {
  ...
  _value.setMilliseconds( value.getMilliseconds() + milliseconds );
  return _value;
}

export function addDays(value: Date, days: number) {
  const _value = new Date(value);
  _value.setDate( value.getDate() + days );
  return _value;
}
export function truncateDateMax(value: Date) {
  const _value = addDays(truncateDateMin(value), 1);
  const r = addMilliseconds(_value,-1);
  return r;
}
export function truncateDateMin(value: Date) {
  const _value = new Date(value);
  const r = new Date(_value.getFullYear(), _value.getMonth(), _value.getDate(), 0, 0, 0, 0);
  return r;
}
